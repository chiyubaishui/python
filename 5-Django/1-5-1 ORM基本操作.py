...
if __name__ == '__main__':
    ...
    # # 查询所有的人
    # ret = models.Person.objects.all()  #是一个对象列表
    # print(ret)	
	# print(ret[0])
	# print(ret[0].name)
    # # get查询，返回的就是一个对象，返回结果有且只有一个，如果符合筛选条件的对象超过一个或者没有都会抛出错误。
    ...
    # # values 返回一个包含某个字段的QuerySet对象，里面都是字典。 不写字段名，默认查询所有字段
    # ret = models.Person.objects.values("name", "birthday")
    # print(ret)
    # print("values_list".center(80, "*"))
    # # values_list 返回一个包含某个字段的QuerySet对象，里面都是元祖。 不写字段名，默认查询所有字段
    ...
    # 正向查询
    # 基于对象 跨表查询
    # book_obj = models.Book.objects.all().first() #一个对象
	# book_obj = models.Book.objects.get(id=1) #这个结果也是一个对象
	# book_obj = models.Book.objects.filter(id=1) #这个结果就不是一个对象了，它是一个queryset，可以当做类一样接各种方法
    # ret = book_obj.publisher  # 和我这本书关联的出版社对象，在book表中是通过字段publish与出版社publish表建立外键关系的
    # print(ret, type(ret)) #是一个对象
    # ret = book_obj.publisher.name  # 和我这本书关联的出版社对象的属性
    # print(ret, type(ret)) #<class 'str'>

    # 查询id是1的书的出版社的名称
    # 利用双下划线 跨表查询    
    # ret = models.Book.objects.filter(id=1).values_list("publisher__name") # 双下划线就表示跨了一张表
    # print(ret)

    # 反向查询
    # 1. 基于对象查询
    # publisher_obj = models.Publisher.objects.get(id=1)  # 得到一个具体的对象
    # # ret = publisher_obj.book_set.all() #obj.表名_set
    # ret = publisher_obj.books.all()  #通过(related_name)
    # print(ret)
    #
    # # 2. 基于双下划线
    # ret = models.Publisher.objects.filter(id=1).values_list("xxoo__title")  #related_query_name
    # print(ret)
